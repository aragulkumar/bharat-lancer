...
const CreateJob = () => {
    ...
    const handleVoiceInput = () => {
        ...
        recognition.onresult = async (event) => {
            ...
            try {
                setLoading(true);
                const response = await jobsAPI.createVoice({ transcript: text });
                const jobId = response.data.data.job._id;
                navigate(`/jobs/${jobId}`);
            } catch (err) {
                setError(err.response?.data?.message || 'Failed to create job from voice');
                setLoading(false);
            }
        };
    ...
    return (
        <div className="container">
            ...
            <div className="create-job-content">
                <Card className="voice-card gradient">
                    <h3>🎤 Try Voice Input!</h3>
                    <p>Speak your job requirements and let AI create the posting for you.</p>
                    <Button
